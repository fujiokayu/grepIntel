...
class TestReportGenerator:
    ...
    def test_load_template(self, mock_file):
        ...
        mock_file.assert_called_once_with(
            os.path.join("mock_templates", "report_template_en.md"),
            "r",
            encoding="utf-8",
        )
    ...
    def test_generate_findings(self):
        ...
        assert (
            len(findings) == 1
        )  # Only one vulnerability (the other is a false positive)
...
"""
        ...
        # Format report
        report = self.report_generator.format_report(
            template_str, analysis_results, statistics
        )
    ...
    @patch.object(ReportGenerator, "load_template")
    @patch.object(ReportGenerator, "format_report")
    def test_generate_with_language_template(
        self, mock_format_report, mock_load_template, mock_file
    ):
