...
class PatternManager:
    ...
    def _load_patterns_from_file(self, file_path: str, target_dict: Dict, key: str) -> None:
        """
    ...
    def load_framework_patterns(self, file_path: str, framework: str, language: str) -> None:
        """
        ...
        if language in self.language_patterns:
            for vuln_type, data in self.language_patterns[language].items():
                if vuln_type not in self.combined_patterns[language]:
                    ...
                    # 既存のパターンリストを更新
                    self.combined_patterns[language][vuln_type]["patterns"] = data["patterns"].copy()
        ...
        for framework, framework_data in self.framework_patterns.items():
            for vuln_type, data in framework_data.items():
                ...
                if vuln_type in self.combined_patterns[language]:
                    ...
                    existing_patterns = set(self.combined_patterns[language][vuln_type]["patterns"])
                    for pattern in data["patterns"]:
                        if pattern not in existing_patterns:
                            self.combined_patterns[language][vuln_type]["patterns"].append(pattern)
                # 新しい脆弱性タイプなら新規追加
                else:
                    self.combined_patterns[language][vuln_type] = {
                        "description": data["description"],
    ...
    def load_patterns_from_directory(self, directory_path: str, is_framework: bool = False) -> None:
        """
