...
class CodeExtractor:
    ...
    def extract(self, scan_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for file_result in scan_results["results"]:
            ...
            file_extraction = self._extract_from_file(
                file_path, language, framework, file_result["matches"]
            )

            if file_extraction:
                extraction_results["files_processed"] += 1
                extraction_results["vulnerabilities_processed"] += len(
                    file_extraction["extractions"]
                )
    ...
    def _extract_context(self, lines: List[str], line_number: int) -> Dict[str, Any]:
        ...
        return {"start_line": start_line, "end_line": end_line, "code": context_code}

    def format_for_llm(
        self, extraction_results: Dict[str, Any], max_tokens: int = 4000
    ) -> str:
        """
