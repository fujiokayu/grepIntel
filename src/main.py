...
def main() -> int:
    ...
    try:
        ...
        logger.warning(
            "python-dotenv not installed. Using environment variables as is."
        )
    ...
    try:
        ...
        languages_dir = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "intelligence", "languages"
        )
        ...
        if args.framework:
            framework_dir = os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "intelligence", "frameworks"
            )
            ...
            if os.path.exists(framework_dir):
                ...
                if os.path.exists(framework_file):
                    ...
                    if framework_language:
                        pattern_manager.load_framework_patterns(
                            framework_file, args.framework, framework_language
                        )
                    else:
                        logger.warning(
                            f"Unknown language for framework: {args.framework}"
                        )
                else:
                    logger.warning(
                        f"Framework pattern file not found: {framework_file}"
                    )
            else:
        ...
        try:
            ...
            logger.info(
                f"Initialized LLM client with provider: {os.getenv('LLM_PROVIDER')}"
            )
            ...
            logger.info(
                "Continuing without LLM analysis. Only pattern matching results will be available."
            )
            llm_client = None
        ...
        # Log results
        logger.info(
            f"Scan completed successfully. Scanned {scan_results['files_scanned']} files."
        )
        logger.info(
            f"Found {scan_results['vulnerabilities_found']} potential vulnerabilities."
        )
        logger.info(
            f"Extracted code from {extraction_results['files_processed']} files."
        )
        ...
        print(
            f"Potential vulnerabilities found: {scan_results['vulnerabilities_found']}"
        )
        ...
        if scan_results["vulnerabilities_found"] > 0:
            ...
            for file_result in extraction_results["results"]:
                ...
                for extraction in file_result["extractions"]:
                    print(
                        f"  - Line {extraction['line_number']}: {extraction['vulnerability_type']}"
                    )
                    print(f"    Description: {extraction['description']}")
                    print(
                        f"    Code Context (lines {extraction['context']['start_line']}-{extraction['context']['end_line']}):"
                    )
                    print(f"    ```")
