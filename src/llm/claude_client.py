...
class ClaudeClient(LLMClient):
    ...
    def analyze(self, prompt: str, max_tokens: int = 4000) -> str:
        ...
        truncated_prompt = truncate_text_to_token_limit(prompt, 100000)  # Claude has a large context limit
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f"Sending prompt to Claude (attempt {attempt + 1}/{max_retries})")
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"Claude rate limit exceeded. Retrying in {retry_delay} seconds...")
    ...
    def get_token_count(self, text: str) -> int:
        ...
        try:
            ...
            logger.warning(f"Error counting tokens with Claude: {str(e)}. Using estimation.")
            return estimate_token_count(text)
