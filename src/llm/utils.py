...
def truncate_text_to_token_limit(text: str, max_tokens: int) -> str:
    ...
    for paragraph in paragraphs:
        ...
        if current_token_count + paragraph_token_count <= max_tokens:
            ...
            if (
                not result
            ):  # If this is the first paragraph, we need to include something
                ...
                if partial_paragraph:
                    ...
                    while (
                        estimate_token_count(truncated_text + "...") > max_tokens
                        and partial_paragraph
                    ):
    ...
    if estimate_token_count(final_text) > max_tokens:
        ...
        for word in words:
            word_token_count = estimate_token_count(word + " ")
            if (
                current_token_count + word_token_count <= max_tokens - 1
...
def format_vulnerability_for_prompt(
    vulnerability: Dict[str, Any], language: str
) -> str:
    """
