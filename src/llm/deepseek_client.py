...
class DeepSeekClient(LLMClient):
    ...
    def analyze(self, prompt: str, max_tokens: int = 4000) -> str:
        ...
        truncated_prompt = truncate_text_to_token_limit(prompt, 16000)  # DeepSeek has a large context limit
        ...
        data = {
            "model": self.model,
            "messages": [
                {"role": "system", "content": "You are a security expert analyzing potential vulnerabilities in source code."},
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f"Sending prompt to DeepSeek (attempt {attempt + 1}/{max_retries})")
                ...
                if response.status_code == 200:
                    ...
                    if attempt < max_retries - 1:
                        logger.warning(f"DeepSeek rate limit exceeded. Retrying in {retry_delay} seconds...")
                        time.sleep(retry_delay)
                        retry_delay *= 2  # Exponential backoff
                    else:
                        logger.error("DeepSeek rate limit exceeded. Maximum retries reached.")
                        raise Exception(f"DeepSeek rate limit exceeded: {response.text}")
                else:
