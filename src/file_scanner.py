...
class FileScanner:
    ...
    def scan(
        self,
        target_path: str,
        language: Union[str, List[str]] = "all",
            ...
            if language == "all":
                ...
                if file_language:
                    ...
                    if file_result:
                        ...
                        self.results["vulnerabilities_found"] += len(
                            file_result["matches"]
                        )
                ...
                if file_ext_language and file_ext_language in language:
                    file_result = self._scan_file(
                        target_path, file_ext_language, framework
                    )
                    if file_result:
                        self.results["files_scanned"] += 1
                        self.results["vulnerabilities_found"] += len(
        ...
        print(
            f"Scan completed: {self.results['files_scanned']} files scanned, {self.results['vulnerabilities_found']} potential vulnerabilities detected"
        )
        ...
        for root, dirs, files in os.walk(directory_path):
            ...
            for file in files:
                ...
                if language == "all":
                    ...
                    if file_language:
                        file_result = self._scan_file(
                            file_path, file_language, framework
                        )
                        if file_result:
                            self.results["files_scanned"] += 1
                            self.results["vulnerabilities_found"] += len(
    ...
    def _scan_file(
        self, file_path: str, language: str, framework: Optional[str]
    ...
    def _match_patterns(
        self, content: str, patterns: Dict[str, Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
