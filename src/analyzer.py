...
class SecurityAnalyzer:
    ...
    def analyze(self, extraction_results: Dict[str, Any], language: str = 'en') -> Dict[str, Any]:
        ...
        for file_result in extraction_results["results"]:
            ...
            logger.info(f"Analyzing {len(extractions)} potential vulnerabilities in {file_path}")
    ...
    def analyze_vulnerabilities_batch(self, extractions: List[Dict[str, Any]], 
                                     file_path: str, language: str,
                                     report_language: str = 'en') -> List[Dict[str, Any]]:
        """
        ...
        for i in range(0, len(extractions), self.batch_size):
            ...
            prompt = self.format_batch_prompt(batch, file_path, language, report_language)
            
            try:
                # Get LLM analysis
                logger.debug(f"Sending batch vulnerability analysis prompt to LLM for {len(batch)} vulnerabilities")
                llm_response = self.llm_client.analyze(prompt)
    ...
    def format_batch_prompt(self, extractions: List[Dict[str, Any]], 
                           file_path: str, language: str,
                           report_language: str = 'en') -> str:
        """
        ...
        template_base += "For each vulnerability, provide your analysis in the following format:\n\n"
    ...
    def parse_batch_response(self, response: str, extractions: List[Dict[str, Any]],
                            file_path: str, language: str,
                            report_language: str = 'en') -> List[Dict[str, Any]]:
        """
        ...
        for i, extraction in enumerate(extractions):
            ...
            if analysis_match:
                ...
                logger.warning(f"Failed to parse batch response for vulnerability {vulnerability_number}. Falling back to individual analysis.")
                individual_analysis = self.analyze_vulnerability(extraction, file_path, language, report_language)
                results.append(individual_analysis)
